...
async function saveAgentToDb(agent) {
    if (!isPostgresConnected || !pool)
        return;
    try {
        await pool.query(`INSERT INTO agents (agent_id, hostname, os_name, os_version, architecture, cpu_model, total_memory_mb, ip_address, status, last_heartbeat)
...
async function saveTelemetryToDb(agentId, payload) {
    if (!isPostgresConnected || !pool)
        return;
    try {
        ...
        if (payload.topProcesses) {
            for (const p of payload.topProcesses) {
                await pool.query(`INSERT INTO process_telemetry (agent_id, pid, parent_pid, name, executable_path, cpu_pct, memory_mb, sha256_hash)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [agentId, p.pid, p.parentPid || null, p.name, p.executablePath, p.cpuPct, p.memoryMb, p.sha256Hash]);
            }
        }
        // Insert Network Telemetry
        if (payload.networkConnections) {
            for (const n of payload.networkConnections) {
                await pool.query(`INSERT INTO network_sockets (agent_id, pid, process_name, protocol, local_address, local_port, remote_address, remote_port, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [agentId, n.pid, n.processName, n.protocol, n.localAddress, n.localPort, n.remoteAddress, n.remotePort, n.status]);
            }
        }
        // Insert File Events
        if (payload.fileEvents) {
            for (const f of payload.fileEvents) {
                await pool.query(`INSERT INTO file_events (agent_id, file_path, action, file_size_bytes)
           VALUES ($1, $2, $3, $4)`, [agentId, f.filePath, f.action, f.fileSizeBytes]);
            }
        }
    }
    catch (err) {
...
async function saveAlertToDb(alert) {
    if (!isPostgresConnected || !pool)
        return;
    try {
        await pool.query(`INSERT INTO alerts (alert_id, agent_id, hostname, rule_id, rule_name, severity, details, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       ON CONFLICT (alert_id) DO UPDATE SET status = EXCLUDED.status`, [alert.alertId, alert.agentId, alert.hostname, alert.ruleId, alert.ruleName, alert.severity, alert.details, alert.status, alert.timestamp]);
    }
    catch (err) {

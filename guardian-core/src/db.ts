...
import { AgentRecord, DiscoveredDevice, EDRAlert, EDREvent, FileTelemetry, NetworkTelemetry, ProcessTelemetry, RuleDefinition } from './types.js';

const DB_PATH = path.join(process.cwd(), 'guardian.db');
...
export async function saveAgentToDb(agent: AgentRecord): Promise<void> {
  if (!db) return;
...
export async function loadAgentsFromDb(): Promise<AgentRecord[]> {
  if (!db) return [];
  return new Promise((resolve) => {
...
export async function saveDiscoveredDevicesToDb(devices: DiscoveredDevice[]): Promise<void> {
    ...
    db.serialize(() => {
      ...
      for (const dev of devices) {
        stmt.run(
          ...
          dev.vendorName,
          dev.deviceType,
          JSON.stringify(dev.openPorts),
          dev.riskScore,
          JSON.stringify(dev.detectedThreats),
          dev.lastSeen
        );
      }
...
export async function loadDiscoveredDevicesFromDb(): Promise<DiscoveredDevice[]> {
  if (!db) return [];
  return new Promise((resolve) => {
    db.all(`SELECT * FROM discovered_devices ORDER BY ip_address ASC`, [], (err, rows: any[]) => {
      if (err || !rows) return resolve([]);
...
export async function saveAlertToDb(alert: EDRAlert): Promise<void> {
  if (!db) return;
  return new Promise((resolve) => {
    const stmt = db.prepare(`
